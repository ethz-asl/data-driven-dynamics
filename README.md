...
```
make estimate-model [model=<modeltype>] [config=<config_file_path>] [data_selection=<none|interactive|setpoint|auto>] [selection_variable=<aux1|aux2|aux3|aux4|aux5|aux6>] [plot=<True/False>] [log=<log_file_path>]
```
...
- setpoint: Data is selected based on a certain `manual_control_setpoint` of the `ulog` or `csv` file. The parameter can be specified in the configuration file of the model. Using the `selection_variable` argument, the `manual_control_setpoint` topic that should be used for data selection can be specified (default: `aux1`).
